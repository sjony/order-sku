...
public abstract class Taxi  implements TaxiRun {
    ...
    public Double getRunTotal(Double startTime, Double distance) {
        ...
        //入参校验
        if(null == startTime || null == distance || new Double(0).compareTo(distance) >= 0 || startTime < 0 || startTime > 24) {
            System.out.println("系统异常，上车时间或者里程数错误！");
            System.exit(0);
        }

        Double currentInitiateRate = null;    //当前起步价
        Double currentAfterStartPerKmPrice = null;    //当前3-10公里的价钱
